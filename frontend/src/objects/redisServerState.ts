...
import { RedisDatabaseItem } from '@/objects/redisDatabaseItem'
import { KeyViewType } from '@/consts/key_view_type'
import { RedisNodeItem } from '@/objects/redisNodeItem'
import { decodeRedisKey, nativeRedisKey } from '@/utils/key_convert'
...
export class RedisServerState {
    ...
    databases!: Record<number, RedisDatabaseItem>
    patternFilter!: string | null
    typeFilter!: string | null
    exactFilter!: boolean
    loadingState!: Record<string, unknown>
    viewType!: number
    nodeMap!: Map<string, RedisNodeItem>
    version!: string
    decodeHistory!: Map<string, string[]>
    decodeHistoryLimit!: number
    separator!: string
    ...
    constructor({
        ...
        if (separatorArg != null && separatorArg !== '') {
     ...
     * @return
     */
    tidyNode(key?: string, skipResort?: boolean) {
    ...
    /**
     * Replace tree/list view and browse metadata from GetBrowserSnapshot (backend source of truth).
     * Preserves decodeHistory.
     */
    hydrateFromBrowseSnapshot(snap: Record<string, unknown>) {
        if (typeof snap.db === 'number') {
            this.db = snap.db
        }
        if (typeof snap.reloadKey === 'number') {
            this.reloadKey = snap.reloadKey
        }
        if (typeof snap.viewType === 'number') {
            this.viewType = snap.viewType
        }
        if (typeof snap.version === 'string') {
            this.version = snap.version
        }
        if (typeof snap.separator === 'string' && snap.separator !== '') {
            this.separator = snap.separator
        }
        const filter = snap.filter as Record<string, unknown> | undefined
        if (filter) {
            this.patternFilter = (filter.match as string) ?? null
            this.typeFilter = (filter.type as string) ?? null
            this.exactFilter = filter.exact === true
        }
        const ls = snap.loadingState as Record<string, unknown> | undefined
        if (ls) {
            this.loadingState = {
                ...this.loadingState,
                fullLoaded: ls.fullLoaded === true,
            }
        }
        if (snap.stats != null && typeof snap.stats === 'object') {
            this.stats = snap.stats as Record<string, unknown>
        }
        const dbs = snap.databases as Array<Record<string, unknown>> | undefined
        if (dbs != null) {
            const next: Record<number, RedisDatabaseItem> = {}
            for (const d of dbs) {
                const idx = Number(d.db)
                next[idx] = new RedisDatabaseItem({
                    db: idx,
                    alias: String(d.alias ?? ''),
                    keyCount: Number(d.keyCount ?? 0),
                    maxKeys: Number(d.maxKeys ?? 0),
                })
            }
            this.databases = next
        }

        const keyStruct = snap.keyStruct as unknown[] | undefined
        this.nodeMap.clear()
        const root = new RedisNodeItem({
            key: `${ConnectionType.RedisDB}`,
            label: `db${this.db}`,
            type: ConnectionType.RedisDB,
            db: this.db,
            children: [],
        })
        this.nodeMap.set(`${ConnectionType.RedisDB}`, root)
        if (keyStruct != null && keyStruct.length > 0) {
            root.children = keyStruct.map((raw) => this._nodeFromSnapshot(raw as Record<string, unknown>))
            for (const ch of root.children) {
                this._indexBrowseNode(ch)
            }
        }
    }

    private _nodeFromSnapshot(raw: Record<string, unknown>): RedisNodeItem {
        const chRaw = raw.children as unknown[] | undefined
        let children: RedisNodeItem[] | undefined
        if (chRaw == null) {
            children = undefined
        } else if (chRaw.length === 0) {
            children = []
        } else {
            children = chRaw.map((c) => this._nodeFromSnapshot(c as Record<string, unknown>))
        }
        return new RedisNodeItem({
            key: String(raw.key ?? ''),
            label: String(raw.label ?? ''),
            name: raw.name != null ? String(raw.name) : undefined,
            type: Number(raw.type ?? 0),
            db: Number(raw.db ?? this.db),
            redisKey: raw.redisKey != null ? String(raw.redisKey) : undefined,
            redisKeyCode: raw.redisKeyCode as number[] | undefined,
            keyCount: Number(raw.keyCount ?? 0),
            maxKeys: Number(raw.maxKeys ?? 0),
            isLeaf: raw.isLeaf === true,
            opened: raw.opened === true,
            expanded: raw.expanded === true,
            redisType: raw.redisType != null ? String(raw.redisType) : undefined,
            children,
        })
    }

    private _indexBrowseNode(node: RedisNodeItem) {
        if (node.type === ConnectionType.RedisKey || node.type === ConnectionType.RedisValue) {
            const rk = node.redisKey ?? ''
            this.nodeMap.set(`${node.type}/${rk}`, node)
        }
